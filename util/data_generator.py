...
test_file = f"{fold_path}/test.txt"
train_file = f"{fold_path}/train.txt"
...
with open(f"{fold_path}/test_scores.txt", "wb") as f:
    lines = [(str(x) + "\n").encode() for x in lgbm_model.predict(test_data[0])]
    f.writelines(lines)


with open(f"{fold_path}/model.json", "wb") as f:
